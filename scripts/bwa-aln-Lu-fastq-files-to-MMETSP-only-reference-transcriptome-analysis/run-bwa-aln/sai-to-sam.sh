...
cat ../1st-sai-fq-index.txt | while read sample; do sai=$(echo "$sample" | awk '{print $1}'); fq=$(echo "$sample" | awk '{print $2}'); \
out=$(echo "$sample" | awk '{print $1}' | sed 's\.sai\\'); bwa samse alexandrium-ref-transcriptome.fa "$sai" \
../trimmed-fastq-files/"$fq" > "$out".sam; done
